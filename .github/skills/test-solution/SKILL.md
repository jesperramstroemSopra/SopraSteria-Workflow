...
1. **Implementation progress** — read from `.goals/workflow/implement-plan/` to understand what was changed.
2. **Original plan** — read from `.goals/workflow/create-plan/` for the expected outcomes.
3. **Analysis and grill reports** — read from `.goals/workflow/analyze-project/` and `.goals/workflow/grill-me/` for the original findings.
...
3. **Execute tests.** Run each test case:
   ...
   - For convention checks: compare against `shared/naming-conventions.md`
...
Save to: `.goals/workflow/test-solution/test-report-{timestamp}.md`
