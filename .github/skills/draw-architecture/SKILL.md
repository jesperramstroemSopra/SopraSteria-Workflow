...
3. **Source** — discover from:
   - Existing workflow artifacts in `.goals/workflow/`
...
Save to: `.goals/workflow/draw-architecture/architecture-{scope}-{timestamp}.html`

Also save a companion markdown summary: `.goals/workflow/draw-architecture/architecture-{scope}-{timestamp}.md`
...
- Follow Sopra naming conventions from `shared/naming-conventions.md` when labeling components.
